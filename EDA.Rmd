...
#mathscores <- mathscores %>%  select(-Mjob)
#mathscores <- mathscores %>%  select(-Fjob)
#mathscores <- mathscores %>%  select(-Medu, -Fedu)
...
boostmath <- gbm(G4 ~ ., data = mathtrain, distribution = "multinomial", n.trees = 1000, shrinkage = 0.01)


summary(boostmath)

predictboost = predict(boostmath, newdata = mathtest, n.trees = 1000 ,type = "response")
pred.class <- colnames(predictboost)[apply(predictboost, 1, which.max)]
table(Predicted = pred.class, Actual = mathtest$G4)
```
...
pred <- predict(ofit, newdata = mathtest, type = "class")
table(Predicted = pred, Actual = mathtest$G4)
...
set.seed(67)
#overfitted tree
mathrpart <- rpart(G4 ~ ., data = mathtrain, cp = 10^(-5))
#predict(titanic_tree1)

printcp(mathrpart)
best_cp <- mathrpart$cptable[which.min(mathrpart$cptable[, "xerror"]), "CP"]
rpart.plot(mathrpart)


mathrpart2 <- rpart(G4 ~ ., data = mathtrain, cp = best_cp)

printcp(mathrpart2)

rpart.plot(mathrpart2)

pred <- predict(mathrpart2, newdata = mathtest, type = "class")
table(Predicted = pred, Actual = mathtest$G4)
